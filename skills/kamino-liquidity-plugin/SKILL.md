...
### Install kamino-liquidity-plugin binary + update wrapper (auto-injected)
...
# Download binary to hidden name (.kamino-liquidity-plugin-core)
if [ ! -f "$HOME/.local/bin/.kamino-liquidity-plugin-core" ]; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/okx/plugin-store/releases/download/plugins/kamino-liquidity-plugin@0.1.0/kamino-liquidity-plugin-${TARGET}${EXT}" -o ~/.local/bin/.kamino-liquidity-plugin-core${EXT}
  chmod +x ~/.local/bin/.kamino-liquidity-plugin-core${EXT}
fi

# Generate wrapper script (version check + exec core binary)
cat > ~/.local/bin/kamino-liquidity-plugin << 'WRAPPER_EOF'
#!/bin/sh
PLUGIN_NAME="kamino-liquidity-plugin"
PLUGIN_VERSION="0.1.0"
CHECKER="$HOME/.plugin-store/update-checker.py"
CORE="$(dirname "$0")/.kamino-liquidity-plugin-core"
if [ -f "$CHECKER" ]; then
...
exec "$CORE" "$@"
WRAPPER_EOF
chmod +x ~/.local/bin/kamino-liquidity-plugin
```
...
if [ ! -f "$REPORT_FLAG" ]; then
  ...
  curl -s -X POST "https://plugin-store-dun.vercel.app/install" \
    ...
    -d '{"name":"kamino-liquidity-plugin","version":"0.1.0"}' >/dev/null 2>&1 || true
