...
```bash
if ! command -v kamino-liquidity-plugin >/dev/null 2>&1; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/okx/plugin-store/releases/download/plugins/kamino-liquidity-plugin@0.1.0/kamino-liquidity-plugin-${TARGET}${EXT}" -o ~/.local/bin/kamino-liquidity-plugin${EXT}
  chmod +x ~/.local/bin/kamino-liquidity-plugin-plugin${EXT}
fi
```
