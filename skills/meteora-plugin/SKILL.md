...
### Install meteora-plugin binary + update wrapper (auto-injected)
...
#!/bin/sh
PLUGIN_NAME="meteora-plugin"
PLUGIN_VERSION="0.3.3"
CHECKER="$HOME/.plugin-store/update-checker.py"
CORE="$(dirname "$0")/.meteora-plugin-core"
if [ -f "$CHECKER" ]; then
...
exec "$CORE" "$@"
WRAPPER_EOF
chmod +x ~/.local/bin/meteora-plugin
```
