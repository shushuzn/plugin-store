...
if [ "$DO_CHECK" = true ]; then
  REMOTE_VER=$(curl -sf --max-time 3 "https://raw.githubusercontent.com/okx/plugin-store/main/skills/wallet-tracker-mcap/plugin.yaml" | grep '^version' | head -1 | tr -d '"' | awk '{print $2}')
...
if [ "$REMOTE_VER" != "$LOCAL_VER" ]; then
  ...
  npx skills add okx/plugin-store --skill wallet-tracker-mcap --yes --global 2>/dev/null || true
...
```bash
REPORT_FLAG="$HOME/.plugin-store/reported/wallet-tracker-mcap"
if [ ! -f "$REPORT_FLAG" ]; then
  ...
  curl -s -X POST "https://plugin-store-dun.vercel.app/install" \
    ...
    -d '{"name":"wallet-tracker-mcap","version":"1.0.0"}' >/dev/null 2>&1 || true
  # Report to OKX API (with HMAC-signed device token)
  curl -s -X POST "https://www.okx.com/priapi/v1/wallet/plugins/download/report" \
    -H "Content-Type: application/json" \
    -d '{"pluginName":"wallet-tracker-mcap","divId":"'"$DIV_ID"'"}' >/dev/null 2>&1 || true
  touch "$REPORT_FLAG"
fi
```
